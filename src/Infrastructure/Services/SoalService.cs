...
{
	public class SoalService : AsyncBaseService<Soal>, ISoalService
	{
		...
		#region appgen: add
		public async Task<Soal> AddAsync(Soal entity, CancellationToken cancellationToken = default)
		{
			...
			await _unitOfWork.SoalRepository.AddAsync(entity);
		...
		#region appgen: count
		public async Task<int> CountAsync(ISpecification<Soal> spec, CancellationToken cancellationToken = default)
		{
			return await _unitOfWork.SoalRepository.CountAsync(spec, cancellationToken);
		}
		#endregion

		#region appgen: delete
		public async Task<bool> DeleteAsync(Soal entity, CancellationToken cancellationToken = default)
		{
			_unitOfWork.SoalRepository.DeleteAsync(entity, cancellationToken);
			await _unitOfWork.CommitAsync();
			return true;
		}
		#endregion
		...
		public async Task<Soal> FirstAsync(ISpecification<Soal> spec, CancellationToken cancellationToken = default)
		{
			return await _unitOfWork.SoalRepository.FirstAsync(spec, cancellationToken);
		}
		#endregion

		#region appgen: first or default
		public async Task<Soal> FirstOrDefaultAsync(ISpecification<Soal> spec, CancellationToken cancellationToken = default)
		{
			return await _unitOfWork.SoalRepository.FirstOrDefaultAsync(spec, cancellationToken);
		}
		#endregion

		#region appgen: get by id
		public async Task<Soal> GetByIdAsync(string id, CancellationToken cancellationToken = default)
		{
			return await GetByIdAsync(id, false, cancellationToken);
		}

		private async Task<Soal> GetByIdAsync(string id, bool includeChilds = false, CancellationToken cancellationToken = default)
		{
			var soal = await _unitOfWork.SoalRepository.GetByIdAsync(id, cancellationToken);
			if (soal == null || includeChilds == false)
				return soal;
		...
		public async Task<IReadOnlyList<Soal>> ListAllAsync(List<SortingInformation<Soal>> sorting, CancellationToken cancellationToken = default)
		{
			return await _unitOfWork.SoalRepository.ListAllAsync(sorting, cancellationToken);
		}
		#endregion

		#region appgen: get list
		public async Task<IReadOnlyList<Soal>> ListAsync(
			ISpecification<Soal> spec, 
			List<SortingInformation<Soal>> sorting,
			bool withChilds = false,
			CancellationToken cancellationToken = default)
		{
			var soals = await _unitOfWork.SoalRepository.ListAsync(spec, sorting, cancellationToken);
			if (withChilds && soals?.Count > 0)
			{
				var results = new List<Soal>(soals);
				var soalIds = soals.Select(e => e.Id).ToList();
		...
		public async Task<bool> UpdateAsync(Soal entity, CancellationToken cancellationToken = default)
		{
			...
			await _unitOfWork.SoalRepository.ReplaceAsync(entity, entity.Id, cancellationToken);
			


			// update & commit
			//await _unitOfWork.SoalRepository.UpdateAsync(entity, cancellationToken);
			await _unitOfWork.CommitAsync();
			return true;
		}
		...
		private bool ValidateBase(Soal entity, int? rowNum = null)
		{
		...
		private bool ValidateOnInsert(Soal entity, int? rowNum = null)
		{
			string rowInfo = (rowNum.HasValue) ? $"Baris {rowNum.Value}:" : string.Empty;
		...
		private bool ValidateOnUpdate(Soal entity, int? rowNum = null)
		{
			string rowInfo = (rowNum.HasValue) ? $"Baris {rowNum.Value}:" : string.Empty;
		...
		public string GeneratePdf(Soal entity, CancellationToken cancellationToken = default)
		{
			if (entity == null)
			...
			// read template
			string templateFile = _uriComposer.ComposeTemplatePath("soal.html");
		...
		public async Task<string> GeneratePdfMultiPage(List<string> ids, int? refId = null, CancellationToken cancellationToken = default)
		{
			...
			var items = await this.ListAsync(new SoalFilterSpecification(ids), null, true, cancellationToken);
		...
		public async Task<string> GeneratePdfMultiPageBackgroundProcess(List<string> ids, CancellationToken cancellationToken = default)
		{
			...
			var downloadProcess = new DownloadProcess("soal") { StartTime = DateTime.Now };
		...
		private string MapTemplateValue(string htmlContent, Soal entity)
		{
			...
			CancellationToken cancellationToken = default)
		{
			try
			{
				SoalFilterSpecification filterSpec = null;
				if (!string.IsNullOrEmpty(id))
					filterSpec = new SoalFilterSpecification(id);
				else
					filterSpec = new SoalFilterSpecification(exact)
					{
				...
				if (ExcelMapper.WriteToExcel<Soal>(excelFilename, "soal.json", results) == false)
				{
					if (refId.HasValue)
		...
		public async Task<List<Soal>> UploadExcel(string tempExcelFile, CancellationToken cancellationToken = default)
		{
			var result = ExcelMapper.ReadFromExcel<Soal>(tempExcelFile, "soal.json");
			if (result == null)
			{
			...
			foreach (var item in result)
			{
				var id = item.Id;
				if (!string.IsNullOrEmpty(id))
					await _unitOfWork.SoalRepository.UpdateAsync(item, cancellationToken);
				else
					await _unitOfWork.SoalRepository.AddAsync(item, cancellationToken);
		...
		//private async Task RunMasterDataValidation(List<Soal> result, CancellationToken cancellationToken)
		//{

		//}

		private void SetUploadDraftFlags(List<Soal> result)
		{
			foreach (var item in result)
			{
		...
		{
			//var spec = new SoalFilterSpecification()
			//{
		...
		public async Task<bool> ProcessUploadedFile(IEnumerable<Soal> soals, CancellationToken cancellationToken = default)
		{
			if (soals == null)
				throw new ArgumentNullException(nameof(soals));
		...
		public async Task<string> GenerateUploadLogExcel(CancellationToken cancellationToken = default)
		{
			...
			{
				var ws = package.Workbook.Worksheets.Add("Soal");
				ws.Cells[1, 1].Value = "ID";
				ws.Cells[1, 2].Value = "PK";
				ws.Cells[1, 3].Value = "Konten Soal";
				...
				foreach (var item in draftDatas)
				{
					...
					ws.Cells[row, 3].Value = item.Konten;
        ...
        public async Task<IReadOnlyList<Soal>> ListAsync(SoalFilter filter, CancellationToken token = default)
        {
			return await _unitOfWork.SoalRepository.ListAsync(filter, token);
        }

        public async Task<int> CountAsync(SoalFilter filter, CancellationToken token = default)
        {
			return await _unitOfWork.SoalRepository.CountAsync(filter, token);
        }

        public async Task<Soal> FirstAsync(SoalFilter filter, CancellationToken token = default)
        {
			return await _unitOfWork.SoalRepository.FirstAsync(filter, token);
        }

        public async Task<Soal> FirstOrDefaultAsync(SoalFilter filter, CancellationToken token = default)
        {
			return await _unitOfWork.SoalRepository.FirstOrDefaultAsync(filter, token);
        }
        #endregion
        ...
        //public async Task<Soal> CreateDraft(CancellationToken cancellation)
        //{
        //	var spec = new SoalFilterSpecification()
        //	{
        //		ShowDraftList = BaseEntity.DraftStatus.DraftMode,
        ...
        //		RecordEditedBy = _userName
        //	}.BuildSpecification();
        //	var count = await _unitOfWork.SoalRepository.CountAsync(spec, cancellation);
        //	if (count > 0)
        //		return await _unitOfWork.SoalRepository.FirstOrDefaultAsync(spec, cancellation);
        ...
        //public async Task<Soal> CreateEditDraft(string id, CancellationToken cancellation)
        //{

        //	var count = await this.CountAsync(new SoalFilterSpecification(id), cancellation);
        //	if(count <= 0)
        //	{
        //		AddError($"Data Soal dengan id {id} tidak ditemukan.");
        //		return null;
        //	}
        ...
        //		ShowDraftList = BaseEntity.DraftStatus.DraftMode
        //	}.BuildSpecification();
        //	var previousDraft = await _unitOfWork.SoalRepository.FirstOrDefaultAsync(spec, cancellation);
        //	if (previousDraft != null)
        //		return previousDraft;

        //	// clone data
        //	var cloneResult = await _unitOfWork.SoalRepository.CloneEntity(id, _userName);
        //	if (cloneResult == null)
        //	{
        //		AddError($"Gagal membuat record Soal");
        //		return null;
        //	}

        //	return await _unitOfWork.SoalRepository.GetByIdAsync(cloneResult.Id, cancellation);

        //}
        #endregion

        #region appgen: patch draft
        //public async Task<bool> PatchDraft(Soal soal, CancellationToken cancellationToken)
        //{
        //	var id = soal.Id;
        //	var originalValue = await _unitOfWork.SoalRepository.FirstOrDefaultAsync(
        //		new SoalFilterSpecification(id));
        ...
        //	if (!string.IsNullOrEmpty(soal.Konten)) originalValue.Konten = soal.Konten;
        ...
        //	await _unitOfWork.SoalRepository.UpdateAsync(originalValue, cancellationToken);
        //	await _unitOfWork.CommitAsync();
        //	return true;
        //}
        #endregion
        ...
        //		// header
        //		destinationRecord.Konten = recoveryRecord.Konten;


        //		await _unitOfWork.SoalRepository.UpdateAsync(destinationRecord, cancellationToken);
        ...
        //	// save ke database
        //	await _unitOfWork.SoalRepository.UpdateAsync(recoveryRecord, cancellationToken);
        //	await _unitOfWork.CommitAsync();
        ...
        //	var datas = await _unitOfWork.SoalRepository.ListAsync(spec, null, cancellationToken);
